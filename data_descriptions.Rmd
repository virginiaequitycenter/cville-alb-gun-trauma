...
The data below comes from news outlets, the Virginia Department of Health, and local and national law enforcement agencies. To keep insights consistent across these jurisdictions we need clearly defined terminology. 
...
group_by(yr = year(arrest_datetime), grp) %>%
  ...
  geom_col() +
  labs(title = "Arrests for Violent Gun-Related Crimes",
       x = "Year",
       y = "Number of Arrests",
...
fai_age %>%
  ...
  reactable(
    ...
    highlight = TRUE,
    defaultPageSize = 11) %>%
    add_legend(fai_age, col_name = 'rate_of_firearm_deaths_per_100k_pop', title = 'Rate of Firearm Deaths per 100k Pop', footer = 'Years: 2018 - 2022', colors = c("#FFFFFF", "#FA8C00"), bins = 9)
...
Exclusions: because the data is voluntarily submitted, it may not be all-inclusive. 

</details>
<br>

## Total Crimes vs. Incidents

```{r ucr_totals}

# should we just focus on incidents and drop crimes? Or vice versa?
...
ucr %>% 
  ...
  labs(x = "Year",
       ...
       title = "All Crimes and Incidents Involving Firearms") +
...
ucr %>% 
  ...
  labs(x = "Year",
       y = "Crime Rate per 100k",
       title = "All Youth Crimes and Incidents Involving Firearms",
       caption = "Note: youth = under 18 years old") +
  scale_color_manual(values = c("#232D4B", "#F8BE3D", "#007BAB"), 
                    name = "Region",
                    breaks = c("virginia", "charlottesville", "albemarle"),
                    labels= c("Virginia", "Charlottesville", "Albemarle"))
...
ucr %>% 
  ...
  left_join(pops) %>%
  mutate_at(vars(matches('n_')), ~ . / est_pop_district * 1e4 ) %>%
  pivot_longer(cols = c(n_juvenile, n_adult)) %>%
