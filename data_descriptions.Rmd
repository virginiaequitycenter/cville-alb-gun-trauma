...
Crime data for Virginia are pulled from the FBI National Incident-Based Reporting System (NIBRS) database. The data contains summaries of all [crimes](#crime) where a firearm was used in Virginia, Albemarle County, and Charlottesville City between 2016 - 2023. Each row represents a different type of crime for each year and each column represents crime counts and rates. 
...
ucr2 <- read_csv("data/ucr2.csv")
pops2 <- read_csv("data/pops2.csv")

ucr2 %>%
  mutate_at(vars(matches('n_')), ~ . / est_pop_district * 1e4 ) -> tmp2

ggplot(tmp2, aes(year, n_total, color = district, group = district)) +
  geom_line(size = 1.5) +
  labs(x = "Year",
  ...
  scale_color_manual(values = c("#232D4B", "#F8BE3D", "#007BAB"), 
    ...
    geom_label(data = filter(tmp2, year == 2023),
...
ggplot(tmp2, aes(year, n_juvenile, color = district, group = district)) +
  geom_line(size = 1.5) +
  labs(x = "Year",
       y = "Crime Rate per 100k Population",
...
ucr2 %>%
  mutate_at(vars(matches('n_')), ~ . / est_pop_district * 1e4 ) %>%
  pivot_longer(cols = c(n_juvenile, n_adult)) -> tmp2
...
ggplot(tmp2, aes(year, value, color = district, group = district)) +
  geom_line(size = 1.5) +
  facet_wrap(~name,
             labeller = labeller(name = age_labs),
             scales = "free_y") +
...
ggplot(theft, aes(year, n_stolen, colour = district)) +
  geom_line(linewidth = 1.5) +
  labs(x = "Year",
