...
arrests %>%
  group_by(id) %>%
  summarise(charges = n()) %>%
  mutate(grp = case_when(charges == 1 ~ "1", 
                         charges %in% 2:4 ~ "2-4",
                         TRUE ~ "5 or more")) %>%
  group_by(grp) %>%
  summarise(charges = sum(charges)) %>%
  ungroup() %>%
  mutate(lbl = scales::percent(charges/sum(charges))) %>%
  ggplot(aes(x = "", y = charges, fill = grp, label = lbl)) +
  geom_col() +
  geom_text(position = "stack", vjust = 2) +
  labs(title = "Distribution of Gun-Related Arrests",
       y = "Number of Charges",
       x = "n = 233") +
  scale_fill_discrete(name = "Charges per Person") 
